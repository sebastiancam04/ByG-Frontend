  ...
  const onSubmit = async (data: LoginForm) => {
    ...
    try {
      ...
      if (response.token) {
        ...
        // ✅ REDIRECCIÓN ARREGLADA: Todos van a la raíz del sitio
        setTimeout(() => {
          router.push("/");
          router.refresh();
        }, 1000);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden font-sans">
        ...
        <Card className="shadow-2xl border-0 bg-white/95 backdrop-blur-sm transition-all duration-300">
                ...
                >
                  <div className="space-y-2">
                    <Label
                      ...
                      className={cn(
                        "text-slate-700 font-medium",
                        errorsReset.codigo && "text-red-500",
                      )}
                    >
                  ...
                  <div className="space-y-2">
                    <Label
                      ...
                      className={cn(
                        "text-slate-700 font-medium",
                        errorsReset.nuevaPassword && "text-red-500",
                      )}
                    >
                      Nueva Contraseña
                    </Label>
                    ...
                    {errorsReset.nuevaPassword && (
                      <p className="text-xs text-red-500 font-bold">
                        {errorsReset.nuevaPassword.message}
                      </p>
                    )}
                  </div>
                  <div className="flex flex-col gap-3 pt-2">
          ...
          <div className="p-6 pt-0 text-center border-t border-slate-100 mt-2">
            <p className="text-xs text-slate-400 mt-4">
              Plataforma de uso exclusivo para personal autorizado de ByG
              Ingeniería.
            </p>
          </div>
        </Card>
      ...
      <div className="absolute bottom-6 text-white/30 text-xs">
        © {new Date().getFullYear()} ByG Ingeniería
      </div>
    </div>
  );
}
